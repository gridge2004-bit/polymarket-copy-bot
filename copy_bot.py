...
def get_best_price(token_id: str, side: str):
    try:
...
def execute_trade(trade: dict, bet_usdc: float):
    ...
    if live_price is None:
        log.warning("  ✗ Could not fetch live price — skipping")
        return
    ...
    try:
        ...
        if not PRIVATE_KEY:
            ...
            return
        client = ClobClient(host=CLOB_HOST, key=PRIVATE_KEY, chain_id=137, signature_type=0)
        client.set_api_creds(client.derive_api_key())
        order_args = OrderArgs(token_id=token_id, price=live_price, size=bet_usdc, side=BUY if side == 'BUY' else SELL)
...
def poll():
    ...
    for t in reversed(new_trades):
        ...
        log.info(f"U0001f3af NEW TRADE │ {"{"}t.get('side','?'){"}"} {"{"}t.get('outcome','?'){"}"} │ their_size={their_size:.0f{"}"} → our_bet={bet{"}"} │ {"{"}ago{"}"}s ago │ {"{"}t.get('title','')[:50]{"}"}")
        if their_size == 0:
...
def main():
    ...
    log.info(f"  Mode   : {'{'}'U0001f534 DRY RUN (no real orders)' if DRY_RUN else 'U0001f7e2 LIVE — REAL ORDERS ENABLED'{'}'}")
    log.info("═" * 44)
